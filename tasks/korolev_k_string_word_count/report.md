# Подсчёт количества слов в строке Королёв Кирилл

- Student: Королёв Кирилл, group 3823Б1ПР2  
- Technology: SEQ | MPI  
...
Коммуникация:
- Все процессы получают свой сегмент строки (строка передаётся одинаковая всем процессам из-за простой природы задачи).
- Каждый процесс вычисляет количество слов в локальном сегменте.
...
- Отсутствие лишних пересылок данных — строка не рассылается частями, т.к. символов мало, а MPI-коммуникации дороже.
